...
async function emitFile(path2, content) {
  try {
    let dirName = (0, import_path.dirname)(path2);
    const isDirExist = await (0, import_promises.access)(dirName).then(() => true).catch(() => false);
    if (!isDirExist) {
      await (0, import_promises.mkdir)(dirName, { recursive: true });
      await emitFile(path2, content);
    } else {
      await (0, import_promises.writeFile)(path2, content);
    }
  } catch (error) {
    console.log(error);
  }
...
async function getContentFromCache(cache, path2, readFileOpts = null) {
  let content;
  if (!cache.has(path2)) {
    content = await (0, import_promises.readFile)(path2, readFileOpts);
...
function removeCommentsFromCode(code) {
  const comments = [];
  import_acorn.default.parse(code, {
    ecmaVersion: 2020,
    onComment: comments,
    sourceType: "module"
  });
...
function extractWebAccessibleResources(code) {
...
var SERVICE_WORK_DEV_PATH = "sw-dev.js";
var stubId = "/@crx/stub";
...
var ManifestProcessor = class {
  ...
  watchPackageJson(input) {
    if (!input)
      return;
    const watcher = (0, import_rollup.watch)({ input });
...
${outDir}/\x1B[32m${item.js[index]}\x1B[`);
    ...
    if ((_a = manifest.action) == null ? void 0 : _a.default_popup) {
      manifest.action.default_popup = (0, import_path6.basename)(manifest.action.default_popup);
    }
    if (manifest.devtools_page) {
      manifest.devtools_page = (0, import_path6.basename)(manifest.devtools_page);
    }
    if (manifest.options_page) {
      manifest.options_page = (0, import_path6.basename)(manifest.options_page);
    }
    if ((_b = manifest.options_ui) == null ? void 0 : _b.page) {
      manifest.options_ui.page = (0, import_path6.basename)(manifest.options_ui.page);
    }
    if ((_c = manifest.sandbox) == null ? void 0 : _c.pages) {
      manifest.sandbox.pages = manifest.sandbox.pages.map(
        (page) => (0, import_path6.basename)(page)
      );
    }
...
async function getAllFilePaths(directoryPath) {
  try {
    const files = await (0, import_promises2.readdir)(directoryPath);
    const filePaths = [];
    for (const file of files) {
      const filePath = import_path7.default.join(directoryPath, file);
...
async function generateLocales(context, srcPath) {
  let localesDir = import_path7.default.join(srcPath, "_locales");
  if (import_fs3.default.existsSync(localesDir)) {
    ...
    for (const fileFullPath of fileFullPaths) {
      context.addWatchFile(fileFullPath);
      const relativePath = import_path7.default.relative(srcPath, fileFullPath);
...
function crxMV3(options = {}) {
  ...
  let srcDir = (0, import_path8.dirname)(manifest);
  let config;
  ...
  return {
    ...
    async options({ input, ...options2 }) {
      ...
      if (Array.isArray(buildInput)) {
        ...
        for (const item of [
          ...
          const name = (0, import_path8.basename)(item, (0, import_path8.extname)(item));
          entryObj[name] = (0, import_path8.resolve)(srcDir, item);
        }
    ...
    async buildStart() {
      ...
      await generateLocales(this, (0, import_path8.join)(config.root, srcDir));
    },
    transform(code, id) {
