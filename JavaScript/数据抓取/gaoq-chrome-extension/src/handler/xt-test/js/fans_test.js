...
function extractFansData(data) {
    ...
    data.distributions.forEach(distribution => {
        ...
        switch (distribution.type_display) {
            case '年龄分布':
                result.ageDistribution = list;
                break;
            case '性别分布':
                result.genderDistribution = list;
                break;
            case '设备品牌分布':
                result.deviceBrandDistribution = list;
                break;
            case '省份分布': // 或者你可以用"城市分布"或其他地理位置
                result.regionDistribution = list;
                break;
            default:
                break;
        }
...
const extractedData = extractFansData(fansData);
