...
import { PgyCommonHandler } from "./pgy/pgy.common"
...
import { ByShopCheckExistsHandler } from "./by/by.shop.search"
...
export default function (version: string) {
    let whiteList = [
        ...
        "ecom/captain/selection_pool/product_list?query_info", // 百应 店铺id补充
    ]
    ...
    if (/https:\/\/pgy\.xiaohongshu\.com/.test(href)) {
        new PgyCommonHandler(context, options);
    }
    ...
    } else if (/https:\/\/buyin\.jinritemai\.com\/dashboard\/goodsCooperation\/product-details/.test(href)) {
        // debugger;
    ...
    } else if (/https:\/\/buyin\.jinritemai\.com\/dashboard\/goodsCooperation\/product-saving/.test(href)) {
        options.tag = Tag.ByProduct;
        console.log("ByProduct",options)
        // debugger;
        return new ByShopCheckExistsHandler(context, options);
    }
    //
